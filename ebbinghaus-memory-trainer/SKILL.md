...
**知识型记忆库（应知应会模式，2026-09 起）：** 适合公司战略、党建知识、制度条文等非编码类内容。与编码库（key→item 单向映射）不同，每条同时存两套内容：
```json
{
  "key": "3+2目标",
  "original": "3是围绕公司收入、利润和员工收入三大目标，2是做诚信经营和结构质态",
  "method": "3个'收'：公司收、利润收、员工收；2个'守'：守诚信、守质态"
}
```
- `original`（原文）：完整原表述，细节（如括号备注）不丢
- `method`（记忆口诀）：助记方法，如"长综融市"、"政大核看"等
- 通用脚本显示逻辑用 `display_text()`：有 `original` 字段的项显示 `**key** original — method`；无 `original` 的编码库项保持原格式 `**编码** 物品 — 记忆方法`，不影响旧库
- 键命名策略：层级内容拆成"总纲 + 分项"多条（如 党建总纲→四个意识/四个自信/两个确立/两个维护 各自独立成条），每条都学，彼此互为对照复习
- **渐进学习模式（先整体后细节，2026-09-01 起）：** 知识型库（公司战略/马斯克/新疆旅行）不用"拆小碎片"，而是 `daily_new=1` + items 数组按 **总纲→分项** 顺序排列，脚本每天只取未学第一条。Day1 背整体概念（如 公司战略总纲），Day2 起每天深入一条（3大目标→2个坚持→4个坚持→…）。中间层条目（如 3+2目标、453主线）内容并入总纲后从 items 删除，避免与分项重复。⚠️ 脚本 `get_new_items` 的 count 必须从 `settings.daily_new` 读取（早期硬编码5导致 daily_new=1 不生效，2026-09-01 修复）。数字编码库等原子化内容维持 daily_new=5。
...
- 用户偏好：不同主题内容（如马斯克学习、数字编码）分开推送，不要混在一个 tracker。**例外（2026-09-01）：用户明确要求时允许合并——把「新疆路线记忆（吾龟的奇幻西游记12节点）」并入马斯克库 tracker（`ebbinghaus_memory_tracker_musk.json`，name 改为「马斯克五步法 + 新疆路线记忆」，cron 名同步更新），知识型条目用 key=谐音字（吾/偷窥/天/赛/伊/那/八/毒/酷/金沙滩香梨/回），original=地名，method=故事句子。旧新疆考试 cron（D+4/D+7/D+15）已废弃，统一走艾宾浩斯库**
...
## 长期巩固机制（每15天，2026-09-01 起）

**触发条件：条目走完标准复习全部间隔**（review_history 中 interval 覆盖 [1,2,4,7,15,30] 全部）后，进入长期巩固池，防止遗忘。

- 计算：`calc_longterm_review()`（通用脚本内），按**自然日**判断——距上次复习 ≥15 天的池内条目出现在推送的 `🧠 长期巩固记忆（每15天）` 区块
- 上次复习基准日：item 的 `longterm_last_review` 字段优先，否则取 review_history 最后一条 date
- 用户测试通过后，由交互流程写入 `longterm_last_review = 当天`，重置15天周期，循环往复（类似标准复习的 unreviewed 判定）
- 验证：模拟数据测试用例见 `/tmp/test_longterm.py`（A=30天前出现、B=14天不出现、C=恰好15天出现、D=未走完间隔不进入、E=无字段回退历史日期）

示例 item 字段：`"longterm_last_review": "2026-09-01"`
